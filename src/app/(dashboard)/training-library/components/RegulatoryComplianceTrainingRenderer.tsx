...
interface RegulatoryComplianceTrainingRendererProps {
  moduleId: string;
...
export function RegulatoryComplianceTrainingRenderer({
  moduleId,
  module,
  onComplete,
  onProgress,
  deepLink,
  onDeepLinkChange,
}: RegulatoryComplianceTrainingRendererProps) {
    ...
    return (
      <Card className="border border-blue-200 bg-blue-50">
        <CardHeader>
          ...
          <CardDescription>Key references and compliance artifacts.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...diagrams, ...infographics, ...images].map((item, index) => (
            <div key={`${item.id || item.title || index}`} className="rounded-lg border border-blue-100 bg-white p-4">
              <h4 className="text-sm font-semibold text-blue-900">{item.title || "Reference"}</h4>
              <p className="text-xs text-blue-700 mt-2">{item.description}</p>
            </div>
          ))}
          {style ? (
    ...
    <div className="space-y-6">
      <Card className="border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <CardContent className="p-8 space-y-4">
          <Badge className="bg-blue-600 text-white">Regulatory Brief</Badge>
          ...
          {content.hook.statistic && (
            <div className="rounded-lg border border-indigo-200 bg-white/70 p-4 text-sm text-indigo-900">
              {content.hook.statistic}
            </div>
          )}
      ...
      {content.learningOutcomes.length > 0 && (
        <Card className="border border-slate-200">
          <CardHeader>
            ...
            <CardDescription>Demonstrate regulatory readiness and evidence.</CardDescription>
          </CardHeader>
      ...
      <div className="space-y-6">
        <Card className="border border-blue-200 bg-white">
          <CardHeader>
            ...
            <CardDescription>Regulatory guidance in practice</CardDescription>
          </CardHeader>
            ...
            <div className="flex flex-wrap gap-2">
              {content.lessons.map((lesson, index) => (
                <Button
                  ...
                  className={index === currentLessonIndex ? "bg-blue-600 hover:bg-blue-700" : "border-blue-200"}
                  ...
                  {examples.map((example, index) => (
                    <li key={index} className="text-sm text-indigo-800">
                      {typeof example === "string" ? (
                        ...
                        <div>
                          <p className="font-semibold text-indigo-900">{example.title}</p>
                          {example.description ? <p>{example.description}</p> : null}
                          {example.outcome ? <p className="text-indigo-700">Outcome: {example.outcome}</p> : null}
                        </div>
                      )}
                    </li>
                  ))}
    ...
    <div className="space-y-6">
      <Card className="border border-indigo-200 bg-indigo-50">
        <CardHeader>
          ...
          <CardDescription>Apply regulatory judgement.</CardDescription>
        </CardHeader>
      </Card>
    ...
    <div className="space-y-6">
      <Card className="border border-slate-200">
        <CardHeader>
          ...
          <CardDescription>Confirm compliance competency.</CardDescription>
        </CardHeader>
      </Card>
    ...
    <div className="space-y-6">
      <Card className="border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <CardContent className="p-8 text-center">
          <h2 className="text-2xl font-bold text-slate-900">Compliance Verified</h2>
          <p className="text-sm text-slate-600 mt-2">You completed {content.title}.</p>
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div>
              <div className="text-2xl font-semibold text-blue-700">{content.estimatedDuration} min</div>
              <div className="text-xs text-slate-500">Estimated time</div>
            </div>
            <div>
              <div className="text-2xl font-semibold text-blue-700">{calculateAssessmentScore()}%</div>
      ...
      <Card className="border border-slate-200">
        ...
        <CardContent className="flex items-center justify-between gap-4 flex-wrap">
          <div className="text-sm text-slate-600">Evidence your regulatory competency.</div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border border-blue-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <Badge className="bg-blue-600 text-white">Regulatory Compliance</Badge>
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-indigo-500">{content.difficulty}</Badge>
          </div>
        </div>
      </div>
      ...
      <Card className="border border-slate-200">
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-slate-800">Regulatory Progress</span>
          ...
          <div className="flex justify-between text-xs text-slate-500 mt-2">
            <span className={currentStage === "hook" ? "text-blue-600 font-semibold" : ""}>Brief</span>
            <span className={currentStage === "content" ? "text-blue-600 font-semibold" : ""}>Sessions</span>
            <span className={currentStage === "practice" ? "text-blue-600 font-semibold" : ""}>Practice</span>
            <span className={currentStage === "assessment" ? "text-blue-600 font-semibold" : ""}>Assessment</span>
            <span className={currentStage === "summary" ? "text-blue-600 font-semibold" : ""}>Summary</span>
          </div>
        </CardContent>
      </Card>
