...
interface CustomerProtectionTrainingRendererProps {
  moduleId: string;
...
export function CustomerProtectionTrainingRenderer({
  moduleId,
  module,
  onComplete,
  onProgress,
  deepLink,
  onDeepLinkChange,
}: CustomerProtectionTrainingRendererProps) {
    ...
    return (
      <Card className="border border-emerald-200 bg-emerald-50">
        <CardHeader>
          ...
          <CardDescription>Outcome-focused references and evidence prompts.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...diagrams, ...infographics, ...images].map((item, index) => (
            <div key={`${item.id || item.title || index}`} className="rounded-lg border border-emerald-100 bg-white p-4">
              <h4 className="text-sm font-semibold text-emerald-900">{item.title || "Reference"}</h4>
              <p className="text-xs text-emerald-700 mt-2">{item.description}</p>
            </div>
          ))}
          {style ? (
  ...
  const renderHookStage = () => (
    <div className="space-y-6">
      <Card className="border border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50">
        <CardContent className="p-8 space-y-4">
          <Badge className="bg-emerald-600 text-white">Customer Outcomes</Badge>
          ...
          {content.hook.statistic && (
            <div className="rounded-lg border border-teal-200 bg-white/70 p-4 text-sm text-teal-900">
              {content.hook.statistic}
            </div>
          )}
          {content.hook.keyQuestion && (
            <div className="rounded-lg border border-emerald-200 bg-white p-4 text-sm text-emerald-800">
              {content.hook.keyQuestion}
            </div>
          )}
        </CardContent>
      ...
      {content.learningOutcomes.length > 0 && (
        <Card className="border border-slate-200">
          <CardHeader>
            ...
            <CardDescription>Delivering good outcomes for customers.</CardDescription>
          </CardHeader>
      ...
      <div className="space-y-6">
        <Card className="border border-emerald-200 bg-white">
          <CardHeader>
            ...
            <CardDescription>Customer journey checkpoint</CardDescription>
          </CardHeader>
                  ...
                  {examples.map((example, index) => (
                    <li key={index} className="text-sm text-teal-800">
                      {typeof example === "string" ? (
                        ...
                        <div>
                          <p className="font-semibold text-teal-900">{example.title}</p>
                          {example.description ? <p>{example.description}</p> : null}
                          {example.outcome ? <p className="text-teal-700">Outcome: {example.outcome}</p> : null}
                        </div>
                      )}
                    </li>
                  ))}
    ...
    <div className="space-y-6">
      <Card className="border border-teal-200 bg-teal-50">
        <CardHeader>
          ...
          <CardDescription>Practice delivering good outcomes.</CardDescription>
        </CardHeader>
      </Card>
        ...
        <Card key={scenario.id} className="border border-slate-200">
          ...
          <CardContent className="space-y-4">
            <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-700">
              {scenario.scenario || scenario.situation || scenario.context || scenario.challenge}
            </div>
            <div className="space-y-2">
              {scenario.options.map((option, index) => {
                const optionId = typeof option === "string" ? `opt-${index}` : option.id || `opt-${index}`;
                const optionText = typeof option === "string" ? option : option.text || "";
                const isSelected = selectedAnswers[scenario.id] === optionId;
                return (
                  <label
                    key={optionId}
    ...
    <div className="space-y-6">
      <Card className="border border-slate-200">
        <CardHeader>
          ...
          <CardDescription>Check your understanding of good outcomes.</CardDescription>
        </CardHeader>
      </Card>
        ...
        <Card key={question.id} className="border border-slate-200">
          ...
          <CardContent className="space-y-3">
            ...
            {question.options.map((option, optionIndex) => {
              ...
              const optionText = typeof option === "string" ? option : option.text || "";
              const isSelected = assessmentAnswers[question.id] === optionId;
              return (
                <label
                  key={optionId}
                  className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer ${
            ...
            {assessmentAnswers[question.id] && question.explanation && (
  ...
  const renderSummaryStage = () => (
    <div className="space-y-6">
      <Card className="border border-emerald-200 bg-gradient-to-r from-emerald-50 to-sky-50">
        <CardContent className="p-8 text-center">
          <h2 className="text-2xl font-bold text-slate-900">Outcome Achieved</h2>
          <p className="text-sm text-slate-600 mt-2">You completed {content.title}.</p>
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div>
              <div className="text-2xl font-semibold text-emerald-700">{content.estimatedDuration} min</div>
              <div className="text-xs text-slate-500">Estimated time</div>
            </div>
            <div>
              <div className="text-2xl font-semibold text-emerald-700">{calculateAssessmentScore()}%</div>
      ...
      <Card className="border border-slate-200">
        ...
        <CardContent className="flex items-center justify-between gap-4 flex-wrap">
          <div className="text-sm text-slate-600">Evidence your training completion.</div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500">
        <a href="/training-library" className="hover:text-emerald-600 transition-colors">
          Training Library
        </a>
        <span>/</span>
        ...
        <span>/</span>
        <span className="text-emerald-600 font-medium">{stageLabels[currentStage]}</span>
      </nav>

      <div className="rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <Badge className="bg-emerald-600 text-white">Customer Protection</Badge>
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-teal-500">{content.difficulty}</Badge>
          </div>
        </div>
      </div>
      ...
      <Card className="border border-slate-200">
        <CardContent className="p-5">
          ...
          <div className="flex justify-between text-xs text-slate-500 mt-2">
            <span className={currentStage === "hook" ? "text-emerald-600 font-semibold" : ""}>Story</span>
            <span className={currentStage === "content" ? "text-emerald-600 font-semibold" : ""}>Sessions</span>
            <span className={currentStage === "practice" ? "text-emerald-600 font-semibold" : ""}>Practice</span>
            <span className={currentStage === "assessment" ? "text-emerald-600 font-semibold" : ""}>Assessment</span>
            <span className={currentStage === "summary" ? "text-emerald-600 font-semibold" : ""}>Summary</span>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <Button
          onClick={nextStage}
          disabled={!canProceed()}
          className={canProceed() ? "bg-emerald-600 hover:bg-emerald-700" : "bg-slate-300 cursor-not-allowed"}
        >
