...
interface OperationalRiskTrainingRendererProps {
  moduleId: string;
...
export function OperationalRiskTrainingRenderer({
  moduleId,
  module,
  onComplete,
  onProgress,
  deepLink,
  onDeepLinkChange,
}: OperationalRiskTrainingRendererProps) {
    ...
    return (
      <Card className="border border-slate-200 bg-slate-50">
        <CardHeader>
          ...
          <CardDescription>Operational maps and resilience cues.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...diagrams, ...infographics, ...images].map((item, index) => (
            <div key={`${item.id || item.title || index}`} className="rounded-lg border border-slate-200 bg-white p-4">
              <h4 className="text-sm font-semibold text-slate-900">{item.title || "Reference"}</h4>
  ...
  const renderHookStage = () => (
    <div className="space-y-6">
      <Card className="border border-slate-200 bg-gradient-to-r from-slate-50 to-amber-50">
        <CardContent className="p-8 space-y-4">
          <Badge className="bg-slate-800 text-white">Control Room Briefing</Badge>
          ...
          {content.hook.statistic && (
            <div className="rounded-lg border border-amber-200 bg-white/70 p-4 text-sm text-amber-900">
              {content.hook.statistic}
            </div>
          )}
      ...
      {content.learningOutcomes.length > 0 && (
        <Card className="border border-slate-200">
          <CardHeader>
            ...
            <CardDescription>What this module prepares you to evidence.</CardDescription>
          </CardHeader>
    ...
    <div className="space-y-6">
      <Card className="border border-amber-200 bg-amber-50">
        <CardHeader>
          ...
          <CardDescription>Simulate disruption and response.</CardDescription>
        </CardHeader>
      </Card>
    ...
    <div className="space-y-6">
      <Card className="border border-slate-200">
        <CardHeader>
          ...
          <CardDescription>Confirm readiness for operational risk scenarios.</CardDescription>
        </CardHeader>
      </Card>
            ...
            {question.options.map((option, optionIndex) => {
              ...
              return (
                <label
                  key={optionId}
                  className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer ${
                    isSelected ? "border-slate-600 bg-slate-100" : "border-slate-200 hover:border-slate-300"
                  }`}
                >
                  <input
                    type="radio"
    ...
    <div className="space-y-6">
      <Card className="border border-slate-200 bg-gradient-to-r from-slate-50 to-amber-50">
        <CardContent className="p-8 text-center">
          <h2 className="text-2xl font-bold text-slate-900">Resilience Verified</h2>
          <p className="text-sm text-slate-600 mt-2">You completed {content.title}.</p>
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div>
              <div className="text-2xl font-semibold text-slate-700">{content.estimatedDuration} min</div>
              <div className="text-xs text-slate-500">Estimated time</div>
            </div>
            <div>
              <div className="text-2xl font-semibold text-slate-700">{calculateAssessmentScore()}%</div>
      ...
      <Card className="border border-slate-200">
        ...
        <CardContent className="flex items-center justify-between gap-4 flex-wrap">
          <div className="text-sm text-slate-600">Evidence your resilience training.</div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <Badge className="bg-slate-800 text-white">Operational Risk</Badge>
      ...
      <Card className="border border-slate-200">
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-slate-800">Control Room Status</span>
          ...
          <div className="flex justify-between text-xs text-slate-500 mt-2">
            <span className={currentStage === "hook" ? "text-slate-800 font-semibold" : ""}>Brief</span>
            <span className={currentStage === "content" ? "text-slate-800 font-semibold" : ""}>Sessions</span>
            <span className={currentStage === "practice" ? "text-slate-800 font-semibold" : ""}>Simulations</span>
            <span className={currentStage === "assessment" ? "text-slate-800 font-semibold" : ""}>Assessment</span>
            <span className={currentStage === "summary" ? "text-slate-800 font-semibold" : ""}>Summary</span>
          </div>
        </CardContent>
      </Card>
